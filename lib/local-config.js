...
var LOCAL_HOME = process.env.jspmConfigFile || process.cwd();
var dprepend = require('./common').dprepend;

// global config - automatically created and loaded on startup
exports.registries = [];
var localConfigFile = LOCAL_HOME + path.sep + '.jspmrc';

function save() {
    try {
        fs.writeFileSync(localConfigFile, JSON.stringify(exports.config, null, 2));
    } catch (e) {
        throw 'Unable to write local configuration file\n' + e.stack;
    }
}
if (fs.existsSync(localConfigFile)) {
    try {
        exports.config = JSON.parse(fs.readFileSync(localConfigFile).toString() || '{}');
    } catch (e) {
        exports.config = {};
        ui.log('err', 'Unable to read local configuration file.\nEnsure you have read and write access to %' + localConfigFile + '%.');
        process.exit(1);
    }
} else {
    exports.config = {};
